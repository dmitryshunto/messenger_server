...
class TokenService extends BaseService_1.BaseHandler {
    ...
    saveToken(userId, refreshToken) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const tokens = yield this.findItems(config_1.table_names['token'], 'userId', userId);
                if (tokens.length) {
                    yield connection.query(`UPDATE ${config_1.table_names['token']} SET refreshToken = ? WHERE userId = ?`, [refreshToken, userId]);
                }
                else
                    yield connection.query(`INSERT INTO ${config_1.table_names['token']} SET ?`, { userId, refreshToken });
    ...
    removeToken(refreshToken) {
        return __awaiter(this, void 0, void 0, function* () {
            const connection = yield this._createConnection();
            yield connection.query(`DELETE FROM ${config_1.table_names['token']} WHERE refreshToken = ?`, [refreshToken]);
            yield connection.end();
        });
    }
}
