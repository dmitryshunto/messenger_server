...
const TokenHandlers_1 = require("./TokenHandlers");
const userDto_1 = require("../dto/userDto");
const BaseHandler_1 = require("./BaseHandler");
class UserService extends BaseHandler_1.BaseHandler {
    constructor() {
        super();
        this.mailServer = new MailHandlers_1.MailServer();
        this.tokenService = new TokenHandlers_1.TokenService();
    }
    createUser(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const logins = yield this.findItems(connection, config_1.table_names['user'], 'login', login);
                if (logins.length)
                    return res.status(400).json({ message: "The user with the same login already exists!" });
                const emails = yield this.findItems(connection, config_1.table_names['user'], 'email', email);
                if (emails.length)
                ...
                let [user] = yield this.findItems(connection, config_1.table_names['user'], 'login', login);
                yield this.mailServer.sendActiovationMessage(email, `${config_1.SERVER_URL}users/activate/${activationLink}`);
    ...
    authorizeUser(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const users = yield this.findItems(connection, config_1.table_names['user'], 'login', login);
    ...
    activateUser(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let users = yield this.findItems(connection, config_1.table_names['user'], 'activationLink', activationLink);
                if (!users.length)
    ...
    refresh(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let tokenFromDb = yield this.tokenService.findItems(connection, config_1.table_names['token'], 'refreshToken', refreshToken);
                if (!userData || !tokenFromDb.length)
                    return res.status(400).json({ message: 'Authorization error!' });
                let users = yield this.findItems(connection, config_1.table_names['user'], 'login', userData.login);
                yield connection.end();
                let user = users[0];
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (login)
                    ...
                    users = yield this.findItems(connection, config_1.table_names['user']);
                let data = [];
